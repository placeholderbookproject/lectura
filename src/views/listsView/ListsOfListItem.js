...
const ListsListItem = (props) => {
    const {img, list_name, list_description, url, list_id, list_created, list_modified, user_name/*, fav, dis*/} = props.list_data
    ...
    const listInteraction = (btn) => {
        if (list_id&&user_id) {
            const input = {type:btn.name, list_id:list_id, user_id:user_id, delete:interactions&&interactions[btn.name]}
            updateListInteraction(input).then(() => {
                ...
                if(["like","dislike"].includes(btn.name)&&interactions[toCheck]){
                    ...
                    console.log({[toCheck]:!interactions[toCheck]})
    ...
    return (
        <div className="list-item">
            ...
            <div className="list-header">
                <h3 className="list-title"><a onClick={()=>navigate(`/lists/${props.tab}/${list_url}`)}>{list_name}</a></h3>
