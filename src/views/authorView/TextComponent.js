...
const TextComponent = props => {
    const {lang, text_id, userData, setTextName, text, setText} = props.properties
    const [q, setQ] = useState();
    const defaultTabs = {"Text Info":true, "Sources": true, "Lists":false};
    const [tabOpen, setTabOpen] = useState({...defaultTabs})
    const tabs = [{tabName:"Text Info", component:<><TextTable properties={{setQ, lang, id:text_id, userData, text, setText, setTextName}}/>{q&&<WikiExternalsList q_number={q} language={lang.value}/>}</>}
