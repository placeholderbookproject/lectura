...
const AuthorTexts = (props) => {
    const {author, language, handleClick, userData, setUserData} = props
    const {author_birth_year, author_death_year, author_name, author_q, author_id} = author
    const [wikiTextdata, setWikiTextdata] = useState(); //wiki
    const [storedtexts,setStoredtexts] = useState(); //database
    ...
    useEffect (() => 
        {if(author&&author_q) {
            fetchDataEffect({setData:setStoredtexts, id:author_id, type:'texts', by: "author", user_id:userData.user_id})();
